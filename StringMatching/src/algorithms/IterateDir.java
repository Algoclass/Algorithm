...
public class IterateDir {
	public static void main(String... args) {
		...
		if (potentialPlagiarisedFile.isDirectory()) {
			File[] filesPotentiallyPlagiarised = potentialPlagiarisedFile
					.listFiles();
	...
	public static void showTestFile(File documentCorpus, File testFile) {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(fileToTest))) {
			...
			br.close();
			System.out
					.println("Enter number for the respective algorithm to run on above test file.");
			...
			case 1:
				...
				for (int i = 0; i < fileData.length; i++) {
					...
					System.out.println("Paragraph wise Test File Data: "
							+ fileData[i]);
					if (documentCorpus.isDirectory()) {
						readCorpusDirectory(documentCorpus.listFiles(),
								fileData[i], true, false, false, false, false);
					} else {
						readCorpusFile(documentCorpus, fileData[i], true,
			...
			case 2:
				...
				for (int i = 0; i < fileData.length; i++) {
					fileData[i] = fileData[i].trim().replaceAll("\\s+", " ");
					System.out.println("Sentence wise Test File Data: "
							+ fileData[i]);
					if (documentCorpus.isDirectory()) {
						readCorpusDirectory(documentCorpus.listFiles(),
								fileData[i], false, true, false, false, false);
					} else {
						readCorpusFile(documentCorpus, fileData[i], false,
			...
			case 3:
				...
				for (int i = 0; i < fileData.length; i++) {
					...
					if (documentCorpus.isDirectory()) {
						readCorpusDirectory(documentCorpus.listFiles(),
								fileData[i], false, false, true, false, false);
					} else {
						readCorpusFile(documentCorpus, fileData[i], false,
			...
			case 4:
				...
				for (int i = 0; i < fileData.length; i++) {
					...
					if (documentCorpus.isDirectory()) {
						readCorpusDirectory(documentCorpus.listFiles(),
								fileData[i], false, false, false, true, false);
					} else {
						readCorpusFile(documentCorpus, fileData[i], false,
			...
			case 5:
				...
				for (int i = 0; i < fileData.length; i++) {
					...
					if (documentCorpus.isDirectory()) {
						readCorpusDirectory(documentCorpus.listFiles(),
								fileData[i], false, false, false, false, true);
					} else {
						readCorpusFile(documentCorpus, fileData[i], false,
	...
	public static void readCorpusFile(File fileInCorpus, String testPattern,
			Boolean lcss, Boolean naive, Boolean kmp, Boolean boyerMoore,
			Boolean rabinKarp) {
		System.out.println("Corpus File: " + fileInCorpus.getName());
		try (BufferedReader br = new BufferedReader(
				new FileReader(fileInCorpus))) {
			StringBuilder sb = new StringBuilder();
			String line;
			while ((line = br.readLine()) != null) {
				sb.append(line);
			...
			if (lcss == true) {
				...
				for (int i = 0; i < fileInCorpusData.length; i++) {
					fileInCorpusData[i] = fileInCorpusData[i].trim()
							.replaceAll("\\s+", " ");
					System.out.println("Paragraph wise Test File Data: "
							+ fileInCorpusData[i]);
					Lcss lcssObj = new Lcss();
					lcssLength = lcssLength
							+ lcssObj.search(fileInCorpusData[i], testPattern)
									.length();
				}
			...
			if (naive == true) {
				...
				for (int i = 0; i < fileInCorpusData.length; i++) {
					fileInCorpusData[i] = fileInCorpusData[i].trim()
							.replaceAll("\\s+", " ");
					System.out.println("Sentence wise Train File Data: "
							+ fileInCorpusData[i]);
					NaiveString naiveStringObj = new NaiveString();
					// naiveStringObj.search(fileInCorpusData[i].toCharArray(),
					// testPattern.toCharArray());
				}
				// TODO
			}
			if (kmp == true) {
				...
				for (int i = 0; i < fileInCorpusData.length; i++) {
					...
					System.out.println("Sentence wise Test File Data: "
							+ fileInCorpusData[i]);
					// Kmp kmpObj = new Kmp();
					// kmpObj.search(fileInCorpusData[i].toCharArray(),
					// testPattern.toCharArray());
				}
				// TODO
			}
			if (boyerMoore == true) {
				...
				for (int i = 0; i < fileInCorpusData.length; i++) {
					...
					// boyerMooreObj.search(fileInCorpusData[i].toCharArray(),
					// testPattern.toCharArray());
				}
				// TODO
			}
			if (rabinKarp == true) {
				...
				for (int i = 0; i < fileInCorpusData.length; i++) {
					...
					// rabinKarpObj.search(fileInCorpusData[i].toCharArray(),
					// testPattern.toCharArray());
				}
				// TODO
			}
	...
	public static void showTestDirectory(File documentCorpus,
			File[] testDirectory) {
	...
	public static void readCorpusDirectory(File[] fileInCorpus,
			String testPattern, Boolean lcss, Boolean naive, Boolean kmp,
			Boolean boyerMoore, Boolean rabinKarp) {
		for (File fileToRead : fileInCorpus) {
			if (fileToRead.isDirectory()) {
				System.out.println("Corpus Directory: " + fileToRead.getName());
				readCorpusDirectory(fileToRead.listFiles(), testPattern, lcss,
						naive, kmp, boyerMoore, rabinKarp);
			} else {
				readCorpusFile(fileToRead, testPattern, lcss, naive, kmp,
