...
public class NaiveString {
	...
	public static ArrayList<String> computeAlgo(File documentCorpus, File testFile){
		...
		naive_list.add("Total time to run the algorithm"+(end-start) +" miliseconds");
		return naive_list;
	}
	...
	static void search(char text[] ,char pattern[], int file_number,ArrayList<String> corpus) {
	        ...
	        if (j == M) { // if pattern[0...M-1] = txt[i, i+1, ...i+M-1]
	        	...
	        	for(int k=0;k<corpus.size();k++)
	        	{
	        		if(corpus.get(k).contains(patt))
	        		{
	        			...
	        			naive_list.add("Pattern found in File "+file_number +" at para"+k +"("+sub+")" + "for sententence (" +patt.substring(0,20)+")");
	        		}
	        	}
